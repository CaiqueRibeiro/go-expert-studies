...
func (fps *FormasPagamentoService) VerificarFormasPagamento(cliente Cliente, valorEntrada float64, valorPedido float64) []string {
	...
	bloqueado := verPedidoBloqueado(pedidosCliente)
	if bloqueado {
		panic("verificarFormasPagamento: Cliente com pedido bloqueado")
	}
	...
	inadimplente := verPagamentoInadimplente(pagamentos)
	if inadimplente {
		panic("verificarFormasPagamento: Cliente com pagamento inadimplente")
	}
...
func verPagamentoInadimplente(pagamentos []Pagamento) bool {
	for _, pagamento := range pagamentos {
		if pagamento.Inadimplente {
			return true
		}
	}
	return false
...
func verPedidoBloqueado(pedidos []Pedido) bool {
